...
character( len= *), parameter :: rcs_id = &
   '$Id: Array.f90,v 1.1 2012/05/23 15:22:52 dnagle Exp $'
...
!  reversion in the format control starts a new line

   write( unit= output_unit, fmt= '( a)') 'Complete array a in column major storage order'
   write( unit= output_unit, fmt= '( f4.0)') a
...
! note that dimension of b is compatible
   ...
   write( unit= output_unit, fmt=  '( a)') 'Array section a( 1: m, 1: m)'
   write_cols_b1: do i = 1, m
      write( unit= output_unit, fmt= '( "b( ",i2,", :)", 4( 1x, f4.0))') i, b( i, :)
   end do write_cols_b1

! note that dimension of b is compatible

   b = a( m: 2*m-1, m: 2*m-1)
   write( unit= output_unit, fmt= '( a)') 'Array section a( m: 2*m-1, m: 2*m-1)'
   write_cols_b2: do i = 1, m
      write( unit= output_unit, fmt= '( "b( ",i2,", :)", 4( 1x, f4.0))') i, b( i, :)
   end do write_cols_b2
