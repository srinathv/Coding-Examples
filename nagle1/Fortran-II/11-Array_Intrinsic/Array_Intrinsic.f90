...
!  write diagnostic information about array pack and unpack
!  compiles with
...
character( len= *), parameter :: rcs_id = &
   '$Id: Array_Intrinsic.f90,v 1.1 2012/05/23 15:26:44 dnagle Exp $'
...
!  declare arrays
   ...
   integer, dimension( n) :: a, a_pack, vector
...
!    write all elements
   ...
   a_pack = pack( a, iand( a, 1) == 1, vector)
!    write all elements
   write( unit= output_unit, fmt=  '( "pack( a, a is odd, 99) ", ( 10 i3))') a_pack

   a_pack = -a_pack

!    write all elements
   write( unit= output_unit, fmt=  '( "unpack( -a_pack, a is odd, a) ", ( 10 i3))') unpack( a_pack, iand( a, 1) == 1, a)
