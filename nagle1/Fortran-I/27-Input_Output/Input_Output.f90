...
!  use the command line get file names to open files and use stream access
!  compiles with
...
character( len= *), parameter :: rcs_id = &
   '$Id: Input_Output.f90,v 1.1 2012/05/22 18:25:48 dnagle Exp $'
...
!  open the input file
   ...
   open( newunit= read_unit, file= input_name, status= 'old', action= 'read', access= 'stream', form= 'formatted', &
...
!  open the output file
   ...
   open( newunit= write_unit, file= output_name, status= 'replace', action= 'write', access= 'stream', form= 'formatted', &
         iostat= io_status, iomsg= message)
...
!  get the number of images and which image this one is
      ...
      read( unit= read_unit, fmt= string_fmt, pos= ipos, iostat= io_status, iomsg= message) buffer
...
!  write the new value of the target
      ...
      write( unit= write_unit, fmt= string_fmt, pos= ipos) buffer

!  get next position

      inquire( unit= read_unit, pos= ipos)
