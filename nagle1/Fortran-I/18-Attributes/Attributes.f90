...
!  write more information about allocatables, pointers and targets
!  compiles with
...
character( len= *), parameter :: rcs_id = &
   '$Id: Attributes.f90,v 1.1 2012/05/22 18:15:40 dnagle Exp $'
...
character( len= *), parameter :: status_fmt = '( a, 1x, l1)'
...
!  declare an integer target
   ...
   integer, allocatable, target :: integer_alloc
...
!  check association status
   ...
   write( unit= output_unit, fmt= status_fmt) 'is pointer associated?', associated( integer_ptr)
...
!  check association status
   ...
   write( unit= output_unit, fmt= status_fmt) 'is pointer associated?', associated( integer_ptr, integer_alloc)
...
!  but it should be deallocated only if its currently allocated
   ...
   if( allocated( integer_alloc) ) deallocate( integer_alloc)
