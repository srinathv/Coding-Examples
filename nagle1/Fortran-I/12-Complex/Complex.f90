...
!  write diagnostic information about the default complex
!  compiles with
!  gfortran -std=f2008 -Wall -fcheck=all Complex.f90 -o Complex
!  executes with
!  ./Complex
...
character( len= *), parameter :: rcs_id = &
   '$Id: Complex.f90,v 1.1 2012/05/22 18:09:51 dnagle Exp $'

!  use an explicit format to write the diagnostic information

character( len= *), parameter :: label_operators_fmt = '( a15, 3( 2x, "(", f10.5, ",", f10.5, ")", :))'
...
complex, parameter :: minus_one = exp( i * pi)

!  a pair of test values

   complex :: a = ( 3.141592654, -2.718281828), b = ( 2.0, 1.0)
...
!  write a constant
   ...
   write( unit= output_unit, fmt= '( a, 2x, "(", f10.6, ",", f10.6, ")" )') 'minus one (the hard way)', minus_one
