...
character( len= *), parameter :: rcs_id = &
   '$Id: Procedures.f90,v 1.1 2012/05/22 18:30:47 dnagle Exp $'
...
!  smooth the values
      ...
      field% values( :, :, to) = smooth_step( field% values( :, :, from) )

return

end subroutine smooth

!  find the max abs difference between to and from

pure function max_error( a) result( maxabs)
real :: maxabs
...
continue
   ...
   maxabs = maxval( abs( a% values( :, :, 1) - a% values( :, :, 2) ))
...
pure function smooth_step( a) result( once)

real, dimension( :, :), intent( in) :: a

real, dimension( size( a, dim= 1), size( a, dim= 2)) :: once
...
!  smooth one time
   ...
   once = 0.25 * ( eoshift( a, shift= 1, dim= 1) &
                 + eoshift( a, shift= -1, dim= 1) &
                 + eoshift( a, shift= 1, dim= 2) &
                 + eoshift( a, shift= -1, dim= 2) )
